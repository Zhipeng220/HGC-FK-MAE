...
class FT_Processor(Processor):
    ...
    def train(self, epoch):
        ...
        for data, label in loader:
            ...
            # forward
            output = self.model(data)  # ✅ 正确
            target = label  # target 就是 label
    ...
    def test(self, epoch):
        ...
        for k in self.arg.show_topk:
            if k == 1:
                self.show_best(k)  # 关键修复：使用 show_best 来设置 self.current_result
            else:
                self.show_topk(k)  # 对于 top-5 等，继续使用 show_topk
